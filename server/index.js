...
app.get('/API', (req, res) => {
  axios({
    method: 'GET',
    //I called this rob host because we will each need to point to our own deployed server from this file. Not sure I know how all three of these will work together, nor if this naming convention is good.
    url: process.env.ROB_HOST_URL + '/products/', //'https://app-hrsei-api.herokuapp.com/api/fec2/hr-rpp/products',
    headers: {
...
app.post('/overview-products', (req, res) => {
   ...
   let productUrl = process.env.ROB_HOST_URL + `/products/${id}`; //`https://app-hrsei-api.herokuapp.com/api/fec2/hr-rpp/products/${id}`
   let headers = {
     'Authorization': process.env.API_KEY
   }
...
app.post('/overview-styles', (req, res) => {
  ...
  //`https://app-hrsei-api.herokuapp.com/api/fec2/hr-rpp/products/${id}/styles`
  // let headers = {
  //   'Authorization': process.env.API_KEY
  // }
  ...
  axios.get(stylesUrl)
  .then(result => {
    console.log('style data at server: ', result.data);
    res.send(result.data)
  })
  .catch((error) => {
    res.sendStatus(404)
